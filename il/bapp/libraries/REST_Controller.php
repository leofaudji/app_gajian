...
abstract class REST_Controller extends CI_Controller {
    ...
    {
        // karena log itu banyak dan harus disimpan maka dibedakan menjadi setiap bulan
        $is_inserted = true;
        $uri = $this->uri->uri_string();
        $mtd = $this->request->method;
        if($mtd !== 'patch'){
            if(strpos($uri, 'ping') === false && strpos($uri, 'init') === false && strpos($uri, 'info') === false && strpos($uri, 'dash/') === false){
    ...
    public function get($key = NULL, $xss_clean = NULL)
    {
        if ($key === NULL)
        {
            return $this->_xss_all($this->_get_args);
        }
    ...
    public function post($key = NULL, $xss_clean = NULL)
    {
        if ($key === NULL)
        {
            return $this->_xss_all($this->_post_args);
        }
    ...
    protected function _log_access_time()
    {
        ...
        return $this->rest->db->update( 'api_logs', $payload, [
            'id' => $this->_insert_id
        ]);
    } 
    ...
    protected function _check_cors()
    {
        ...
        // Tambahkan Credentials untuk allow cookie terbang ke server dari mirza
    ...
    /**
     * ini untuk semua data agar tidak kena xss EDIT DARI MIRZA UNTUK MENEYESUAIKAN SECURITY DARI BPJS 
     * 4 SEPTEMBER 2021
     */
    protected function _xss_all($va){
        foreach($va as $key => $value){
            //$va[ $key ] = str_replace('[removed]', '', $this->security->xss_clean($value));
            $va[ $key ] = $this->security->xss_clean($value);
        }
