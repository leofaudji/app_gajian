...
class Gjdrft extends Bismillah_Controller{
    ...
    public function cetak_get(){
        ...
        if(isset($r)){
            ...
            if($r['jml'] > 0){
                ...
                $alamatkantor   = $this->Bdb->getOne('alamat', 'mst_kantor', array('kode'=>$kode_kantor)) ;   
                $namaperiode    = $this->Bdb->getOne('keterangan', 'gj_periode', array('kode'=>$periode)) ;                
                ...
                if(count($arr['b']) > 0){// generate table                    
                    ...
                    $cetak[]= array('text'=>$alamatkantor, 'fontSize'=>8, 'bold'=>true, 'alignment'=>'left');
                    $cetak[]= chr(13) ; 
                    ...
                    $cetak[]= array('text'=>"Periode : " . $namaperiode, 'fontSize'=>8, 'bold'=>true, 'alignment'=>'center');
                    $cetak[]= chr(13) . chr(13);
    ...
    public function cetakdetil_get(){
        ...
        $s = '';
        $this->gr1_where($bs, $s);
        $this->db->where('m.kode', $kode_kry);                                    
        $db = $this->Bdb->db->select("count(m.id) jml,m.kode,m.nama,m.rekening,m.tgl_masuk,m.golongan,mg.keterangan namagolongan,md.keterangan namadivisi,m.jabatan,mj.keterangan namajabatan")
                            ->from("mst_karyawan m")
                            ->join("gj_golongan mg","mg.kode = m.golongan","left")  
                            ->join("mst_jabatan mj","mj.kode = m.jabatan","left")  
                            ->join("mst_divisi md","md.kode = m.divisi","left")
        ...
        if(isset($r)){
            ...
            if($r['jml'] > 0){
                ...
                $vhead[0][0]= array('text'=>'1. Tambahan', 'width'=>'*', 'bold'=>true, 'alignment'=>'left', 'fontSize'=>10) ; 
                $vhead[0][1]= array('text'=>' ', 'width'=>'*', 'bold'=>true, 'alignment'=>'center', 'fontSize'=>10) ; 
                $vhead[0][2]= array('text'=>'', 'width'=>'10', 'bold'=>true, 'alignment'=>'left', 'fontSize'=>10) ; 
                $vhead[0][3]= array('text'=>'2. Potongan', 'width'=>'*', 'bold'=>true, 'alignment'=>'left', 'fontSize'=>10) ; 
                $vhead[0][4]= array('text'=>' ', 'width'=>'*', 'bold'=>true, 'alignment'=>'center', 'fontSize'=>10) ; 
                

                $dff        = array('text'=>0, 'alignment'=>'right', 'bold'=>true, 'italics'=>true) ;
                $arr        = array('b'=>array(), 'o'=>array(),'tb'=>array(),'to'=>array());

                $arr['datakry']  = array("kode"=>array('width'=>50, 'alignment'=>'left'),
                                         "tanda"=>array('width'=>5, 'alignment'=>'center'),
                                         "keterangan"=>array('width'=>150, 'alignment'=>'left'),
                                         "spasi"=>array('width'=>'*', 'alignment'=>'center'),
                                         "kode1"=>array('width'=>50, 'alignment'=>'left'),
                                         "tanda1"=>array('width'=>5, 'alignment'=>'center'),
                                         "keterangan1"=>array('width'=>150, 'alignment'=>'left')
                                        );

                $arr['o']   = array("tambahan"=>array('width'=>"*", 'alignment'=>'left',  'fontSize'=>8),
                                    "jumlah"=>array('width'=>50, 'alignment'=>'right',  'fontSize'=>8),
                                    "spasi"=>array('width'=>10, 'alignment'=>'left'),
                                    "potongan"=>array('width'=>"*", 'alignment'=>'left',  'fontSize'=>8),
                                    "jumlahp"=>array('width'=>50, 'alignment'=>'right',  'fontSize'=>8)
                                );
                 
                $arr['to']   = array("kettambahan"=>array('width'=>'*', 'bold'=>true, 'alignment'=>'left'),
                                     "saldotambahan"=>array('width'=>50, 'bold'=>true, 'alignment'=>'right'),
                                     "spasi"=>array('width'=>10, 'alignment'=>'left'),
                                     "ketpotongan"=>array('width'=>'*', 'bold'=>true, 'alignment'=>'left'),
                                     "saldopotongan"=>array('width'=>50, 'bold'=>true, 'alignment'=>'right')
                            );

                $arr['tbb']   = array("kettambahan"=>array('width'=>'*', 'bold'=>true, 'alignment'=>'left'),
                            "saldotambahan"=>array('width'=>50, 'bold'=>true, 'alignment'=>'right'),
                            "spasi"=>array('width'=>10, 'alignment'=>'left'),
                            "ketpotongan"=>array('width'=>'*', 'bold'=>true, 'alignment'=>'left',  'fontSize'=>10),
                            "saldopotongan"=>array('width'=>50, 'bold'=>true, 'alignment'=>'right',  'fontSize'=>10)
                   );
                  ...
                  if($key == "D"){
                    ...
                    foreach($value as $key1=>$value1){
                        if($value1['nominal'] > 0){
                            ...
                            $kettambahan              = ($value1['perhitungan'] <> '1') ? $value1['tambahan'] . " (" . $value1['perhitungan'] . " x " . number_format($value1['nominal']) . ")" : $value1['tambahan'] ; 
                    ...
                    foreach($value as $key1=>$value1){
                        if($value1['nominal'] > 0){
                            ...
                            $ketpotongan              = ($value1['perhitungan'] <> '1') ? $value1['potongan'] . " (" . $value1['perhitungan'] . " x " . number_format($value1['nominal']) . ")" : $value1['potongan'] ; 
                ...
                //print_r($data1); 
                $arr['datakryt'][]  = array("kode"=>"Kode Kry","tanda"=>":","keterangan"=>$r['kode'],
                                            "spasi"=>"",
                                            "kode1"=>"Jabatan","tanda1"=>":","keterangan1"=>$r['namajabatan']
                                        );
                $arr['datakryt'][]  = array("kode"=>"Nama","tanda"=>":","keterangan"=>$r['nama'],
                                        "spasi"=>"",
                                        "kode1"=>"Rekening","tanda1"=>":","keterangan1"=>$r['rekening']
                                    );

                $masakerja  = hitung_umur($r['tgl_masuk'] ?? date("Y-m-d")) ;
                $arr['datakryt'][]  = array("kode"=>"Tgl Masuk","tanda"=>":","keterangan"=>date_2d($r['tgl_masuk']),
                                    "spasi"=>"",
                                    "kode1"=>"Masa Kerja","tanda1"=>":","keterangan1"=>$masakerja
                                );

                $arr['b']  = $data1;
                $xlsx['DETIL'] = $data1;   

                $data = array("kettambahan"=>"Total Pendapatan","saldotambahan"=>number_format($tottambahan,0),"ketpotongan"=>"Total Potongan","saldopotongan"=>number_format($totpotongan,0));
                $arr['tb'][]  = $data; 
                $xlsx['DETIL'][] = $data;

                $arr['tbbo'][]  = array("kettambahan"=>"","saldotambahan"=>"","ketpotongan"=>"Total Gaji Bersih","saldopotongan"=>number_format($tottambahan - $totpotongan,0));
                ...
                $arr['ttdo'][]  = array("karyawan"=>"","direktur"=>"Malang, " . date("d-m-Y")) ;
                $arr['ttdo'][]  = array("karyawan"=>"Karyawan","direktur"=>"Bag. Keuangan") ;
                ...
                $arr['ttdo'][]  = array("karyawan"=>"","direktur"=>"") ;
                $arr['ttdo'][]  = array("karyawan"=>"","direktur"=>"") ;
                $arr['ttdo'][]  = array("karyawan"=>"(" . $r['nama'] . ")","direktur"=>$namakantor) ;
                ...
                //if(count($arr['b']) > 0){// generate table                     
                    ...
                    $cetak[] = array('canvas' => array(array('type' => 'line', 'x1' => 0, 'y1' => 5, 'x2' => 515, 'y2' => 5, 'lineWidth' => 1)));
                    $cetak[]= chr(13) ;
                    $cetak[]= pdf_tbl($arr['datakry'], $arr['datakryt'], array('layout'=>'noBorders'), false);
                    $cetak[] = array('canvas' => array(array('type' => 'line', 'x1' => 0, 'y1' => 5, 'x2' => 515, 'y2' => 5, 'lineWidth' => 0.3)));
                    $cetak[]= chr(13);
