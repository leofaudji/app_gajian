...
class Absmet extends Bismillah_Controller{
    ...
    public function gr1_post(){
        ...
        if($this->aruser['lv'] !== "0000"){
            $this->lv_e = boolval(strpos($this->aruser['menu_md5'], md5("absmet.e")));
            $this->lv_d = boolval(strpos($this->aruser['menu_md5'], md5("absmet.d")));
        }
        ...
        if(isset($r)){
            ...
            if($r['jml'] > 0){ 
                ...
                foreach($db->result_array() as $r){
                    ...
                    if($this->lv_e) $r['cmd'] .= '<button type="button" onClick=bo.absmet.edit("'.$r['kode'].'") class="btn btn-default btn-w2gr w-100">Koreksi</button>';
                    if($this->lv_d) $r['cmd'] .= '<button type="button" onClick=bo.absmet.delete("'.$r['id'].'") class="btn btn-danger btn-w2gr w-100">Hapus</button>';
                    $r['cmd'] .= '</div>';
    ...
    public function index_get($kode){
        // koreksi
        $r = $this->Bdb->getOne('*', 'mst_abs_metode', array('kode'=>$kode));
        if(!empty($r)){ 
    ...
    public function hapus_get($id){ 
        // delete
        $lresult = false;
        $r = $this->Bdb->getOne('*', 'mst_abs_metode', array('id'=>$id));
        if(!empty($r)){
          $this->db->where('id', $id);
          $this->db->delete("mst_abs_metode"); 
          $lresult = true;
        }
    ...
    public function index_post($kode){
        ...
        $this->Bdb->upsert('mst_abs_metode', $va, array("kode"=>$kode)) ; 
